...
<figure><img src="../../public/Group.png" alt=""><figcaption><p>设置-模型供应商</p></figcaption></figure>
...
GoModel 在需要模型时，会根据使用场景来选择设置过的默认模型。在 `设置 > 模型供应商` 中设置默认模型。
...
在 GoModel 的 `设置 > 模型供应商` 中设置要接入的模型。
...
在 GoModel 中接入不同类型的模型供应商的方式稍有不同。

**接入自有模型的模型供应商**

接入自有模型的供应商后，GoModel 会自动接入该供应商下的所有模型。

在 GoModel 中设置对应模型供应商的 API key，即可接入该模型供应商。

{% hint style="info" %}
GoModel 使用了 [PKCS1\_OAEP](https://pycryptodome.readthedocs.io/en/latest/src/cipher/oaep.html) 来加密存储用户托管的 API 密钥，每个租户均使用了独立的密钥对进行加密，确保你的 API 密钥不被泄漏。
{% endhint %}
